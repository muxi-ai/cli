...
- `muxi validate` checks agent knowledge sources: `path` XOR `url`, the ten supported remote URL schemes (http, https, s3, gs, az, rsync, rsync+ssh, ftp, sftp, file), and the `agent_tree` block (`regenerate` one of manual/on-source-change/on-formation-load; local sources only). Per-source `retrieval` mode validation is deliberately deferred pending the retrieval-mode consolidation decision.
